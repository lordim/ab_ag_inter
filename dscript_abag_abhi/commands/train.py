...
from ..models.embedding import FullyConnectedEmbed, AbTransformerEncoder
...
def add_args(parser):
    ...
    proj_grp.add_argument(
        ...
        default=256,
        dest="input_dim_ab",
        help="dimension of input language model AbMAP embedding (per amino acid) (default: 256)",
    )
    proj_grp.add_argument(
        "--projection-dim",
        type=int,
...
def train_model(args, output):
    ...
    if args.checkpoint is None:
        ...
        # Create embedding Ab model
        input_dim_ab = args.input_dim_ab
        projection_dim = args.projection_dim
        dropout_p = args.dropout_p
        embedding_model_ab = FullyConnectedEmbed(
            input_dim_ab, projection_dim, dropout=dropout_p
        )
        log("Initializing Ab embedding model with:", file=output)
        log(f"\tprojection_dim: {projection_dim}", file=output)
        log(f"\tdropout_p: {dropout_p}", file=output)

        # Create Ab Transformer Encoder model
        abTransEnc = AbTransformerEncoder(
            input_dim_ab, nheads=16, nencs=1
        )
        log("Initializing Ab Transformer Encoder model...", file=output)
        ...
        # load from a pre-trained D-SCRIPT model
        ...
        state_dict_path = '/net/scratch3/scratch3-3/chihoim/ab_ag_interaction/dscript_abag/dscript/dscript_human_v2.pt'
        state_dict = torch.load(state_dict_path)
        model.load_state_dict(state_dict, strict=False)
        
        # UNCOMMENT LATER!!
        # set dscript-pretrained parameter updates to False
        for name, para in model.named_parameters():
            if 'abTransEnc' in name or 'embedding_ab' in name or 'contact' in name:
                log(f"Training the parameters in {name}", file=output)
                para.requires_grad = True
                continue
            else:
                log(f"Freezing the parameters in {name}", file=output)
    ...
    # OPTIMIZER WITH VARIABLE LEARNING RATE PER LAYER
    # optim = torch.optim.Adam(
    #     [
    #         {"params": model.abTransEnc.parameters(), "lr": 1e-3},
    #         {"params": model.embedding_ab.parameters(), "lr": 1e-3},
    #         {"params": model.contact.parameters(), "lr": 1e-4},
    #     ],
    #     lr=0.0,
    #     weight_decay=wd
    # )
