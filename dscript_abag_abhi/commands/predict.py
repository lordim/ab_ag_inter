...
def main(args):
    ...
    try:
        ...
        if abmapPath is None:
            all_prots = set(pairs.iloc[:, 0]).union(set(pairs.iloc[:, 1]))
        else:
            ag_prots = set(pairs.iloc[:, 0])
            ab_prots = set(pairs.iloc[:, 1])
    ...
    if embPath is None:
        ...
        if abmapPath is None:
            ...
            embeddings = load_hdf5_parallel(embPath, ag_prots)
            embeddings_ab = load_hdf5_parallel(abmapPath, ab_prots)
